...
rustup -y target add wasm32-wasi
...
LLVM_VERSION=$(llvm-ar --version | grep -oP "(?<=LLVM version )([0-9]+\.[0-9]+\.[0-9]+)")
export CMAKE_PREFIX_PATH=/usr/lib/llvm-$LLVM_VERSION/lib/cmake
