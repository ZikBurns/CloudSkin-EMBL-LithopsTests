        ...
        if entry.is_file():
            ...
            for batch_size in batch_sizes:
                ...
                for index in range(0, 1):
                    ...
                    benchmark_result = {'invocation_time':last_end - first_start,  'time_download': time_download, 'time_inference': time_inference, 'predictions':predictions}
                    filename = f"batch_benchmarks/time_results_5_9_23_eu-west/cold/{filename_without_extension}_{batch_size:03d}/{index}.json"
                ...
                for index in range(1, num_experiments):
                    ...
                    last_end = max(end_times)
                    predictions=result_body['predictions']
                    benchmark_result = {'invocation_times':last_end - first_start,  'time_download': time_download, 'time_inference': time_inference, 'predictions':predictions}
                    filename = f"batch_benchmarks/time_results_5_9_23_eu-west/warm/{filename_without_extension}_{batch_size:03d}/{index}.json"
                    os.makedirs(os.path.dirname(filename), exist_ok=True)
                    out_file = open(filename, "w")
                    json.dump(benchmark_result, out_file, indent=6)
                    out_file.close()
