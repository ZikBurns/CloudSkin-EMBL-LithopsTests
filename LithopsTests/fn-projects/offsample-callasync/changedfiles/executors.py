...
class FunctionExecutor:
    ...
    def change_executor_id(self, executor_id:str):
        ...
        logger.debug(f'Function executor for {self.backend} changed ID to: {self.executor_id}')
    def change_storage_to_local(self):
        config = {
            'backend': 'localhost',
            'bucket': 'storage',
            'localhost': {'max_workers': 1, 'runtime': 'python', 'storage_bucket': 'storage',
                       'user_agent': 'lithops/2.9.0', 'worker_processes': 16}
        }
        self.internal_storage = InternalStorage(storage_config)
        self.storage = self.internal_storage.storage
    ...
    def call_async_cnn(
        ...
        # shutil.copytree("/tmp/lithopscnn-object/lithops.jobs/torchs-0", "/tmp/lithops-pepe/storage/lithops.jobs/torchs-0")
        # shutil.copytree("/tmp/lithopscnn-object/lithops.jobs/torchs-0-A000", "/tmp/lithops-pepe/storage/lithops.jobs/torchs-0-A000")
        # shutil.copyfile("/tmp/lithopscnn-object/lithops.jobs/torchs-0-A000-job.json", "/tmp/lithops-pepe/storage/lithops.jobs/torchs-0-A000-job.json")
        #
        # with open("/tmp/lithopscnn-object/job.json", 'r') as file:
        #     json_data = json.load(file)
        #
        # job = SimpleNamespace(**json_data)
        futures = self.invoker.run_job(job)
        self.futures.extend(futures)
