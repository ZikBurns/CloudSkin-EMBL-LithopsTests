...
def get_function_and_modules(job, internal_storage):
    ...
    func_download_end = time.time()
    resultedtime = str(round(func_download_end - func_download_start, 6))
    with open("/tmp/times.txt", 'a+') as f:
        f.write(resultedtime + '\n')
        logger.info(f"Download time: {resultedtime} ")
    with open("/tmp/times.txt", 'r') as file:
        data = file.read().replace('\n', ' ')
        logger.info(f"Download times: {data} ")

    func_download_start = time.time()
    loaded_func_all = pickle.loads(func_obj)
    func_download_end = time.time()
    resultedtime = str(round(func_download_end - func_download_start, 6))
    with open("/tmp/sertimes.txt", 'a+') as f:
        f.write(resultedtime + '\n')
        logger.info(f"Deserialize time: {resultedtime} ")
    with open("/tmp/sertimes.txt", 'r') as file:
        data = file.read().replace('\n', ' ')
        logger.info(f"Deserialize times: {data} ")
