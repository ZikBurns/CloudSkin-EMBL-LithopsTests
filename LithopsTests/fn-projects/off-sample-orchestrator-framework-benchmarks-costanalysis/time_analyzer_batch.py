...
directory_path = 'batch_benchmarks_7077/time_results_2_8_23_time_model_init/warm'
results=[]
for entry in os.scandir(directory_path):
    if entry.is_file():
        ...
        list_time_inference=[]
        list_time_sum = []
        list_time_model_init=[]
        for subentry in os.scandir(entry.path):
            if subentry.is_file():
                print("File:", subentry.name)
                with open(subentry.path, 'r') as file:
                    times = json.load(file)
                    list_time_model_init.append(times["time_model_init"])
                    list_time_total.append(times["invocation_time"])
        ...
        results.append({
            ...
            "time_model_init_mean": np.mean(list_time_model_init),
            "time_model_init_std": np.std(list_time_model_init),
...
print(results)
with open("batch_benchmarks_7077/time_results_2_8_23_time_model_init.json", "w") as file:
    json.dump(results,file,indent=6)

fieldnames = results[0].keys()
file_path = "batch_benchmarks_7077/time_results_2_8_23_time_model_init_warm.csv"
